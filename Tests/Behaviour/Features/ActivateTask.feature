@fixtures
Feature: Activate task

  As a user of Bitzer I want to be able to mark a task as active
  ...
  Scenario: Try to activate a non-existing task
  ...
  Scenario: Try to activate a non-existing task using a constraint check result
  ...
  Scenario: Activate an existing task
    When the command ActivateTask is executed with payload:
      | Key            | Value               |
      | taskIdentifier | "tasky-mc-taskface" |
    Then I expect the task "tasky-mc-taskface" to exist
    And I expect this task to have action status "https://schema.org/ActiveActionStatus"
